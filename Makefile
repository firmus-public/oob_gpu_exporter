...
build-linux-amd64:
	GOOS=linux GOARCH=amd64 go build $(GOFLAGS) -o oob_gpu_exporter-v$(VERSION)-linux-amd64 ./cmd/oob_gpu_exporter

build-linux-arm64:
	GOOS=linux GOARCH=arm64 go build $(GOFLAGS) -o oob_gpu_exporter-v$(VERSION)-linux-arm64 ./cmd/oob_gpu_exporter
